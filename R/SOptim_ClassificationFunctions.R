...
calibrateClassifier <- function(calData,
  ...
  # Check the % number of successfull evaluation rounds, if less than 75% then an error occurrs
  if((sum(is.na(perfStats))/length(dp)) >= 0.75){
    ...
    if(runFullCalibration){
      ...
      # Build utput object data and metadata
      #
...
#' 
#' Produces a summary of classification performance results for a \code{SOptim.Classifier} object.
#' 
#' @param object An object of class \code{SOptim.Classifier} created by \link{calibrateClassifier} with 
#' option \code{runFullCalibration=TRUE}.
#' 
#' @param ... Additional arguments affecting the summary produced.
#' 
#' @return A summary of the object printed to the console.
#' 
...
summary.SOptim.Classifier <- function(object, ...){
  ...
  nr <- nrow(evalMatrix)
  evalMatrix <- rbind(evalMatrix,apply(evalMatrix[-nr,],2,mean),apply(evalMatrix[-nr,],2,sd))
  rownames(evalMatrix) <- c(rownames(evalMatrix)[-c(nr+1,nr+2)],paste("AVG (",object$ClassParams$evalMethod,")",sep=""),
                       paste("ST-DEV (",object$ClassParams$evalMethod,")",sep=""))
   
  cat("\n\n||| SOptim.Classifier object |||\n\n")
   
  cat(".: Classification method:",object$ClassParams$classificationMethod,"\n\n")
  
  cat(".: Classification performance for method ",object$ClassParams$evalMethod,":\n\n",sep="")
  ...
  if(object$ClassParams$nClassType=="multi-class"){
    ...
    # cm <- cbind(cm, user.acc = round(user.acc,3))
    # cm <- rbind(cm, prod.acc = round(prod.acc,3))
    # cm[nc + 1, nc + 1] <- NA
    
    acc <- data.frame(class.label = colnames(cm), 
               User.Accuracy      = round(user.acc,3), 
               Producer.Accuracy  = round(prod.acc,3))
    cat("\n\n")
    print(acc)
  }else{
    acc <- NA
