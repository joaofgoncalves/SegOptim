...
#' 
#' @param trainData Input train data. The input can be a \code{RasterLayer}, \code{SpatialPointsDataFrame}, 
...
#' 
#' @param ... Additional options passed to \code{writeRaster} function. The option \code{datatype} is already 
#' internally set to \emph{INT2U}.
#' 
#' @return A raster object with train segments.
#' 
...
getTrainRasterSegments <- function(trainData, rstSegm, filename = NULL, useThresh = TRUE, thresh = 0.5, 
                                   na.rm = TRUE, dup.rm = TRUE, minImgSegm = 30, ignore = FALSE, verbose = TRUE, ...){
  ...
  rstSegmDF <- data.frame(cell_ID = 1:ncell(rstSegm), SID = raster::values(rstSegm))
  ...
  newRstTrainData <- raster::raster(rstSegm)
  raster::values(newRstTrainData) <- rstSegmDF$train
  
  if(verbose) cat("done.\n\n")
  
  if(!is.null(filename)){
    if(verbose) cat("-> Writing data...\n") ## ---------------------------------------------------------- ##
    raster::writeRaster(newRstTrainData, filename = filename, datatype="INT2U", ...)
...
#' 
#' @param rstSegm A string defining the path to the raster with segment IDs, a \code{rasterLayer} object or 
...
#' 
#' @param ... Additional arguments for \code{\link[raster]{writeRaster}} function except \code{datatype} which is already 
#' internally set to 'INT4U'. 
#' 
#' @details By default the function uses the classifier ran with the 'full' dataset (i.e., no train/test splits) 
#' for making class label predictions. In case of single-class problems the threshold that maximixes the selected 
...
#' 
#' @return An object of class \code{RasterLayer} containing the predicted class labels for 
...
predictSegments <- function(classifierObj, calData, rstSegm, predictFor = "all", 
  ...
  rstSegmDF <- data.frame(cell_ID=1:raster::ncell(rstSegm), SID=raster::values(rstSegm))
  ...
  if(!is.null(filename)){ ## ---------------------------------------------------------------------------- ##
    ...
    #
    raster::writeRaster(newRstPred, filename = filename, datatype="INT4U", ...)
